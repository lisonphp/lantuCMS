...
class category extends admin {
	...
	public function init () {
		$sites = pc_base::load_model('site_model');
		$res_site=$sites->get_one();//是否更新缓存,0为未更新，1为已更新
		if(empty($res_site['olddomain_status']) || empty($res_site['cache_status'])){
			$olddomain_status=$res_site['olddomain_status'];//未更新域名,0为未更新，1为已更新
			$cache_status=$res_site['cache_status'];//是否更新缓存,0为未更新，1为已更新
			$directs=$res_site['domain'].'index.php?m=admin&c=site&a=init';
			showmessage('请先完善信息！',$directs);
			// header("localtion:$aa");exit;
			// include $this->admin_tpl('site_list');exit;
			// echo $res_site['domain'];exit;
		}
		$show_pc_hash = '';
		...
		if(!empty($result)) {
			foreach($result as $r) {
				...
				$r['str_manage'] .= '<a href="?m=admin&c=category&a=edit&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'&type='.$r['type'].'&pc_hash='.$_SESSION['pc_hash'].'">'.L('edit').'</a>';
				$r['str_manage'] .='<a style="margin-left:15px;color:#b02" href="javascript:confirmurl(\'?m=admin&c=category&a=delete&catid='.$r['catid'].'&menuid='.$_GET['menuid'].'\',\''.L('confirm',array('message'=>addslashes($r['catname']))).'\')">'.L('delete').'</a>';
				...
				if($r['type'] =='1'){//单网页 lison
					$r['catname']='<a href="?m=content&c=content&a=add&menuid=822&catid='.$r['catid'].'&pc_hash='.$_GET['pc_hash'].'">'.$r['catname'].'</a>';
				}else{
					$r['catname']='<a href="?m=content&c=content&a=init&menuid=822&catid='.$r['catid'].'&pc_hash='.$_GET['pc_hash'].'">'.$r['catname'].'</a>';
				}
				$r['help']=$r['catdir'];
				$categorys[$r['catid']] = $r;
			}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			$dir=trim($_POST['info']['catdir']);
			if(strtolower($dir) =='wap'){
				showmessage('保留的目录名！');exit;
			}
			...
			//lison 
			$res_site=$this->site->get_one(array('siteid'=>1));
			$setting['template_list']=$res_site['default_style'];
			$_POST['info']['setting'] = array2string($setting);
			// var_dump($_POST['info']);exit;
			...
			$res_site=$this->site->get_one(array('siteid'=>1));
			$res_site=$res_site['default_style'];
			$type = $_GET['s'];
			if($type==0) {
				$exists_model = false;
				$models = getcache('model','commons');	
