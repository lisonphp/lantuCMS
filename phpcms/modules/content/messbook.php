...
class messbook extends admin {
	private $db,$priv_db;
	public $siteid,$categorys;
	public function __construct() {
		parent::__construct();
		$this->db = pc_base::load_model('messbook_model');
	...
	public function messbook() {
		...
		// if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			//只显示最近30天的留言
			$lison_endtime=strtotime(date('Y-m-d',time()));
			$lison_starttime=$lison_endtime - 86400 * 30;
			$where .= " AND `inputtime` > '$lison_starttime'";
			...
			for($i=1;$i<=$workflow_steps;$i++) {
				...
				$workflow_menu .= '<a href="?m=content&c=content&a=&menuid='.$_GET['menuid'].'&catid='.$catid.'&steps='.$i.'&pc_hash='.$pc_hash.'" '.$current.' ><em>'.'待审核'.$newimg.'</em></a><span>|</span>';
			}
			// if($workflow_menu) {//退稿
	...
	public function out_excel(){
		if(isset($_REQUEST['dosubmit'])){
			$excelpassword=trim($_POST['excelpassword']);
			$dbadmin = pc_base::load_model('admin_model');
			$res_admin=$dbadmin->get_one(array('excel_out_password'=>$excelpassword));
			if(!$res_admin){
				showmessage('导出密码不正确！');
			}
			$ids=$_POST['ids'];
			if($ids){
				//有选择的导出
				$idin=implode(',',$ids);
				$where="where status=99 and a.id in($idin)";
			}else{
				//导出最新1个月的
				$yuedate=strtotime(date("Y-m-d",time())) - 86400*30;
				$where="where status=99 and inputtime > $yuedate";
			}
			// print_r($idin);exit;
			...
			$objProps = $objPHPExcel->getProperties();//获取对象属性   
			$objProps->setCreator("lanbox");   //创建者
			$objProps->setLastModifiedBy("lanbox"); //最后改进  
			$objProps->setTitle("lanbox");//标题
			$objProps->setSubject("lanbox");//主题
			$objProps->setDescription("lanbox");//描述  
			$objProps->setKeywords("lanbox");//关键词
			$objProps->setCategory("lanbox");//种类
			$objPHPExcel->setActiveSheetIndex(0);//设置活动对象索引   
			$objActSheet = $objPHPExcel->getActiveSheet();//获取索引 
			
			$objActSheet->setTitle('Sheet1');
		
			$objActSheet->setCellValue(chr(65).'1', '姓名');
			$objActSheet->setCellValue(chr(66).'1', '手机号码');
			$objActSheet->setCellValue(chr(67).'1', '品牌');
			$objActSheet->setCellValue(chr(68).'1', '媒介');
			$objActSheet->setCellValue(chr(69).'1', '微信/QQ');
			$objActSheet->setCellValue(chr(70).'1', '内容');
			$objActSheet->setCellValue(chr(71).'1', '网址URL');
			$objActSheet->setCellValue(chr(72).'1', '省');
			$objActSheet->setCellValue(chr(73).'1', '市');
			$objActSheet->setCellValue(chr(74).'1', 'IP/邮箱');
			$objActSheet->setCellValue(chr(75).'1', '关键字');
			$objActSheet->setCellValue(chr(76).'1', '备注');
			$objActSheet->setCellValue(chr(77).'1', '提交时间');

			// $this->db->set_model(14);
			// $this->db->table_name('messbook');
			// $this->db->table_name = $this->db->table_name.'_data';
			// $list=$this->db->select(array('status'=>99));
			$this->db->query("select * from lt_messbook a left join lt_messbook_data b on a.id=b.id $where");
			$list=$this->db->fetch_array();
			// print_r($list);exit;
			$i = 1;        
			foreach ($list as $k => $v) {//线索备注列表                      
				$i++;
				$objActSheet->setCellValue(chr(65).$i, $v['name']);
				$objActSheet->setCellValue(chr(66).$i, $v['tel']);  
				$objActSheet->setCellValue(chr(67).$i,'');
				$objActSheet->setCellValue(chr(68).$i,$v['60']);
				$objActSheet->setCellValue(chr(69).$i,$v['qq']);
				$objActSheet->setCellValue(chr(70).$i,$v['content']);
				$objActSheet->setCellValue(chr(71).$i,$v['url']);
				$objActSheet->setCellValue(chr(72).$i,$v['sheng']);
				$objActSheet->setCellValue(chr(73).$i,$v['shi']);
				$objActSheet->setCellValue(chr(74).$i,$v['ip']);
				$objActSheet->setCellValue(chr(75).$i,$v['channel'].$v['ziops']);
				$objActSheet->setCellValue(chr(76).$i,$v['beizhu']);
				$objActSheet->setCellValue(chr(77).$i,date('Y-m-d H:i:s',$v['inputtime']));     
			}



			$current_page = intval($_REQUEST['current_page']);//当前页
			$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');//读写操作对象
			ob_end_clean();//清理缓存
			header("Content-Type: application/vnd.ms-excel;");
			header("Content-Disposition:attachment;filename=lanbox_bills_".date('Y-m-d',mktime())."_".$current_page.".xls");
			header("Pragma:no-cache");
			header("Expires:0");
			$objWriter->save('php://output');//以print和echo一样的方式写入到
			// session('export_status', 0);//输出状态
		}else{
			echo '不需要走到这里';exit;
			...
			// extract($info); 
			include $this->admin_tpl('messbook_out_excel');
		}
	}

	public function ajaxshowtel(){
		$id=$_POST['id'];
		$tel_password=$_POST['tel_password'];
		$dbadmin = pc_base::load_model('admin_model');
		$res_admin=$dbadmin->get_one(array('excel_out_password'=>$tel_password));
		if(!$res_admin){
			$arrs['code']=0;
			$arrs['msg']='密码错误！';
		...
		$this->db->set_model(14);
		$res=$this->db->get_one(array('id'=>$id));
		$arrs['code']=1;
		$arrs['txt']=$res['tel'] / 864;
		echo json_encode($arrs);exit;
	}
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($r){
				$tel=$r['tel'] / 864; 
				$r['tel']=substr_replace($tel, '****', 3,4);
			}
			$this->db->table_name = $this->db->table_name.'_data';
			$r2 = $this->db->get_one(array('id'=>$id));
			// print_r($this->db->table_name);exit;
			...
			include $this->admin_tpl('messbook_edit');
		}
	...
	public function ajaxdelete(){
		...
		$dbadmin = pc_base::load_model('admin_model');
		$res_admin=$dbadmin->get_one(array('excel_out_password'=>$deletepassword));
		if(!$res_admin){
			$arrs['code']=0;
			$arrs['msg']='密码错误！';
			echo json_encode($arrs);exit;
