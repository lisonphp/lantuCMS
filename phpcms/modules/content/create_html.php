...
class create_html extends admin {
	...
	public function urlupdate() {//lison
		if(isset($_POST['dosubmit'])) {
			//extract($_POST,EXTR_SKIP);
			$this->url = pc_base::load_app_class('url');
			$this->sitemodel = pc_base::load_model('sitemodel_model');

			$table_res = $this->sitemodel->select('modelid <>10','tablename,modelid');
			// $where = " WHERE status=99";
			$where = "";
			foreach ($table_res as $key => $v) {
				$table_name='lt_'.$v['tablename'];
				$rs = $this->db->query("SELECT * FROM `$table_name` $where");
				$data = $this->db->fetch_array($rs);
				foreach($data as $r) {
					if($r['islink'] || $r['upgrade']) continue;
					//更新URL链接
					// $this->urls($r['id'], $r['catid'], $r['inputtime'], $r['prefix']);
					$urls = $this->url->show($r['id'], 0, $r['catid'], $r['inputtime'], $r['prefix'],'','edit');
					//更新到数据库
					$url = $urls[0];
					// var_dump($url);exit;
					$this->db->set_model($v['modelid']);
					$this->db->update(array('url'=>$url),array('id'=>$r['id']));
				}
			}
			$sites = pc_base::load_model('site_model');
			$sites->update(array('olddomain_status'=>1), array('siteid'=>1));//未更新域名,0为未更新，1为已更新
			// $updatedomian='';//见modules/admin/site.php edit()
			echo '<div style="font-size:26px;">完成</div>';exit;	

		} else {
			// $this->public_cache();
			//Allowed memory size of 134217728 bytes exhausted错误,查找到memory_limit = 128M这一行，将128M改大点，我这里直接是改成了2048M
			// echo 22;exit;
			$show_header = $show_dialog  = '';
			$admin_username = param::get_cookie('admin_username');
			$modelid = isset($_GET['modelid']) ? intval($_GET['modelid']) : 0;
			...
			$tree->init($categorys);
			$string .= $tree->get_tree(0, $str);
			include $this->admin_tpl('urlupdate_lison');
		}
	}
