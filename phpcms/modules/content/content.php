	...
	public function add() {
		if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
			...
			if($category['type']==0) {
				...
				$this->page_history = pc_base::load_model('pagehistory_model');
				...
				if(!isset($_POST['info']['template']) || $_POST['info']['template'] ==''){
					$_POST['info']['template']=date("Ydm",time());
				}
				$_POST['info']['updatetime']=time();
				if($_POST['edit']) {
					$this->page_db->update($_POST['info'],array('catid'=>$catid));
					$catid_history= $this->page_history->insert($_POST['info'],1);
				} else {
				...
				if ($status==99) {
					...
					if (isset($_POST['ids']) && !empty($_POST['ids'])) {
						foreach ($_POST['ids'] as $id) {
							...
							$flag = $catid.'_'.$id;
							// if($setting['presentpoint']>0) {//会员积分lison
