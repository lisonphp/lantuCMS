...
class index {
	...
	public function updateDomain(){
	 	if(isset($_POST['new_domain'])){
	 		 $this->domain_model->update(array('url'=>$_POST['new_domain']),'id=1');
	 		 echo 1;exit;
	 	}
	}

	//此版本服务器上，来自客户端请求是否有更新
	public function check_version(){
		$time=date('Y-m-d H:i',time());
	 	if(isset($_POST['version'])){	
	 		$version = $_POST['version'];
	 		if($version <1.65){//2019-03-20之前的旧版本
	 			echo json_encode(array('up_date'=>'2019-03-20','version'=>'666','versiontxt'=>'有新安装包更新！'));exit;
	 		}
	 		if(!isset($_POST['tplcode'])){
	 			echo '缺少参数！';exit;
	 		}
	 		if(empty(trim($_POST['version'])) || empty(trim($_POST['tplcode']))){
	 			echo '缺少参数哈！';exit;
	 		} 
	 		$tplcode = $_POST['tplcode'];
	 		$tplname = $_POST['tplname'];
	 		if(isset($_POST['url'])){
	 			$urlData = $this->domain_model->get_one(array('url'=>$_POST['url'],'status'=>0),'id');
	 			if(!$urlData) $this->domain_model->insert(array('url'=>$_POST['url'],'tplcode'=>$tplcode,'tplname'=>$tplname,'addtime'=>$time));	 			
	 		}		

	 		//线上版本大于客户端版本，则进行更新
	 		$wheres="version > '$version' and tplcode= '$tplcode'";
		 	$res_version = $this->version_update->get_one($wheres,'version,url,up_date,versiontxt');
		 	if($res_version){
		 		$str='';
		 		$i=1;
		 		$result = $this->version_update->select($wheres,'versiontxt');
		 		foreach ($result as $key => $v) {
		 			$str.=$i.'、'.$v['versiontxt'].'<br>';
		 			$i++;
		 		}
	 			$data=array('up_date'=>$res_version['up_date'],'version'=>$res_version['version'],'versiontxt'=>$str);
	...
	//此版本服务器上，来自客户端请求，返回需要更新的文件URL
	public function check_version_update(){
		if(isset($_POST['version'])){	
	 		$version = $_POST['version'];
 			if($version <1.65){//之前的旧版本,不给更新了，需要下载新的包 2019-03-20 
	 			//http://versioncms.blueto.net/version/lantucms.zip
	 			echo json_encode(array('code'=>0,'message'=>'不支持当前(旧)版本升级,请使用最新的安装包或联系相关技术人员！'));exit;
	 		}
	 		if(!isset($_POST['tplcode'])){
	 			echo '缺少参数tplcode！';exit;
	 		}
	 		if(empty(trim($_POST['version'])) || empty(trim($_POST['tplcode']))){
			 	echo json_encode(array('code'=>0,'message'=>'version(版本号)或tplcode(识别码)不能为空！'));exit;
	 		} 
	 		$tplcode = $_POST['tplcode'];
			$wheres="version > '$version' and tplcode= '$tplcode'";
		 	$res_version = $this->version_update->get_one($wheres,'version,url,up_date,versiontxt,tplcode');
	 		if($res_version){
	 			$data =array();
	 			$file =array();
	 			$result = $this->version_update->select($wheres,'version,url,serverurl,up_date,versiontxt');
	 			foreach($result as $v){
	 				// echo json_encode(111);exit;
	 				if($v['url'] !='' && file_exists($v['serverurl'])){
	 					$file[$v['url']] = file_get_contents($v['serverurl']);
	 					$data[] = $v;
	 				}
	 			}
	 			if(!$file && !$data){
	 				echo json_encode(array('code'=>0,'message'=>'暂无更新哟'));exit;
	 			}
	 			echo json_encode(array('file'=>$file,'data'=>$data));exit;
	 		}else{
	 			echo json_encode(array('code'=>0,'message'=>'暂无更新'));exit;
	 		}
	 	}
