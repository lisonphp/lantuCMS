...
class block_admin extends admin {
	...
	public function __construct() {
		...
		$this->dbtwo = pc_base::load_model('blocktwo_model');
	...
	public function add() {
		$pos = isset($_REQUEST['pos']) && trim($_REQUEST['pos']) ? trim($_REQUEST['pos']) : showmessage(L('illegal_operation'));
	...
	public function block_update() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->update($sql, array('id'=>$id))) {
				//添加历史记录//'data'=>array2string($data)
				$this->history_db->insert(array('blockid'=>$data['id'], 'data'=>$datas, 'creat_at'=>SYS_TIME, 'userid'=>param::get_cookie('userid'), 'username'=>param::get_cookie('admin_username')));
