...
class index {
	function __construct() {	
		...
		$this->sites = $this->site->get_one(array('siteid'=>$this->siteid));
	...
	public function index() {
		$wap=PHPCMS_PATH.'wap';
		if(is_dir($wap)){//存在wap文件夹，将优先访问此
			header("Location: /wap/index.html");exit;
		}
		// var_dump($_GET);exit;
		$dir=trim($_GET['dir']);
		if(isset($_GET['id'])){
		...
		$cate=$this->cate->get_one(array('catdir'=>$dir));
		if(!$cate){
			if(!$dir){//首页
				$this->db = pc_base::load_model('site_model');
				$res_site=$this->db->get_one();
				$SEO = array('site_title'=>trim($res_site['site_title']),'description'=>trim($res_site['description']),'keywords'=>trim($res_site['keywords']));
				$CATEGORYS = getcache('category_content_1','commons');
				// var_dump($cate[17]['ismenu']);exit;
				include template('wap', 'index');exit;
			}else{
		...
		// $CATEGORYS = getcache('category_content_1','commons');
		$setting_cate = string2array($cate['setting']);
		// var_dump($setting_cate);exit;
		if($cate['type'] ==0){//列表模型
			$MODEL = getcache('model','commons');
			$tablename = $this->db->table_name = $this->db->db_tablepre.$MODEL[$modelid]['tablename'];
			if($id){//内容页
				$res_titletxt = $this->db->get_one(array('id'=>$id));
				if(!$res_titletxt){
					header("HTTP/1.1 404 Not Found");  
					header("Status: 404 Not Found");
					header("Location: /404.html");
				}
				$title=$res_titletxt['title'];
				$thumb =$res_titletxt['thumb'];
				$updatetime =date("Y-m-d H:i",$res_titletxt['updatetime']);//更新时间
				$author =$res_titletxt['author'];//作者
				$keywords = $res_titletxt['description'] ? $res_titletxt['description'] : $setting_cate['meta_keywords'];
				$description=$res_titletxt['description'] ? $res_titletxt['description'] : $setting_cate['meta_description'];

				$SEO = array('site_title'=>trim($res_titletxt['title']),'description'=>trim($description),'keyword'=>trim($keywords));
				//上一页
				$previous_page = $this->db->get_one("`catid` = '$catid' AND `id`<'$id' AND `status`=99",'*','id DESC');
				//下一页
				$next_page = $this->db->get_one("`catid`= '$catid' AND `id`>'$id' AND `status`=99",'*','id ASC');
				if(empty($previous_page)) {
					$previous_page = array('title'=>'第一页', 'thumb'=>IMG_PATH.'nopic_small.gif', 'url'=>'javascript:alert(\''.'没有了'.'\');');
				}else{
					$previous_page['url'] =str_replace(stristr($previous_page['url'],'.',true),'http://m',$previous_page['url']);
				}

				if(empty($next_page)) {
					$next_page = array('title'=>'最后一页', 'thumb'=>IMG_PATH.'nopic_small.gif', 'url'=>'javascript:alert(\''.'没有了'.'\');');
				}else{
					$next_page['url'] =str_replace(stristr($next_page['url'],'.',true),'http://m',$next_page['url']);
				}
				
				$this->db->table_name = $tablename.'_data';
				$res_contxt = $this->db->get_one(array('id'=>$id),'content');
				$content =$res_contxt['content'];
				// var_dump($SEO);exit;
				$template=$setting_cate['show_template'];
			}else{//列表页
				$SEO = array('site_title'=>trim($setting_cate['meta_title']),'description'=>trim($setting_cate['meta_description']),'keyword'=>trim($setting_cate['meta_keywords']));

				$wheres=" where catid=".$catid.' and status=99';
				$sql = "select * from $tablename $wheres order by id desc";
				$seget = pc_base::load_model('get_model');
				// $page = intval($_GET['page'])?intval($_GET['page']) :'1';
				$infopage = $seget->multi_listinfo($sql,$page,8); //返回查询结果
				$pages = $seget->pages;//返回分页
				// var_dump($pages);exit;
				$template=$setting_cate['list_template'];
			}
		}else{//单页模型
			$this->page_db = pc_base::load_model('page_model');
			$rpage = $this->page_db->get_one(array('catid'=>$catid));
			// var_dump($setting_cate);exit;
			if($rpage) extract($rpage);
			$title = $setting_cate['meta_title'] ? $setting_cate['meta_title'] : $title;
			$keywords = $setting_cate['meta_keywords'] ? $setting_cate['meta_keywords'] : $keywords;
			$description=$setting_cate['meta_description'] ? $setting_cate['meta_description'] : $title;
			$SEO = array('site_title'=>trim($title),'description'=>trim($description),'keyword'=>trim($keywords));
			$template=$setting_cate['page_template'];
		}
		// echo $template;exit;
		include template('wap', $template);
	}
