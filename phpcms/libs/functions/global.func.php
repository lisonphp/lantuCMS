...
function get_domain_url(){//仅支持二级域名如 a.abc.com
	$p = pc_base::load_model('site_model');
	$r = $p->get_one();
	if(stristr($r['domain'],'http://www.')){
		$res_url['http_wap_url']=str_replace('http://www.', 'http://m.', $r['domain']);
		$res_url['wap_url']=str_replace('http://www.', 'm.', $r['domain']);
		$res_url['wap_url']=str_replace('/', '', $res_url['wap_url']);

		$res_url['http_pc_url']=$r['domain'];
		$res_url['pc_url']=str_replace('http://www.', 'www.', $r['domain']);
		$res_url['pc_url']=str_replace('/', '', $res_url['pc_url']);

		$res_url['http_head']='http://www.';
	}else{
		$strs=explode('.', $r['domain']);
		$res_url['http_wap_url']='http://m.'.$strs[1].'.'.$strs[2];
		$res_url['wap_url']='m.'.$strs[1].'.'.str_replace('/', '', $strs[2]);

		$arr=explode('//',$strs[0]);
		$res_url['http_pc_url']='http://'.$arr[1].'.'.$strs[1].'.'.$strs[2];
		$res_url['pc_url']=$arr[1].'.'.$strs[1].'.'.str_replace('/', '', $strs[2]);

		$res_url['http_head']='http://'.$arr[1].'.';
	}
	return $res_url;
}

function get_domain_url2($domain){//仅支持二级域名如 a.abc.com
	$r['domain']=$domain;
	if(stristr($r['domain'],'http://www.')){
		$res_url['http_wap_url']=str_replace('http://www.', 'http://m.', $r['domain']);
		$res_url['wap_url']=str_replace('http://www.', 'm.', $r['domain']);
		...
		$res_url['nohttp_pc_url']=str_replace('http://www.', '', $r['domain']);
		$res_url['nohttp_pc_url']=str_replace('/', '', $res_url['nohttp_pc_url']);
		...
		$res_url['http_pc_url']='http://'.$arr[1].'.'.$strs[1].'.'.$strs[2];
		$res_url['pc_url']=$arr[1].'.'.$strs[1].'.'.str_replace('/', '', $strs[2]);
		$res_url['nohttp_pc_url']=$strs[1].'.'.str_replace('/', '', $strs[2]);
...
function pages2($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	if(defined('URLRULE') && $urlrule == '') {
		$urlrule = URLRULE;
		$array = $GLOBALS['URL_ARRAY'];
	} elseif($urlrule == '') {
		// $urlrule = url_par('page={$page}');
		$urlrule = url_par2('{$page}');
		// var_dump($urlrule);exit;
	}
	$multipage = '';
	if($num > $perpage) {
		$page = $setpages+1;
...
function url_par2($par, $url = '') {
	...
	if($pos === false) {
		...
		$querystring = http_build_query($query_array).'_'.$par;
		$url = substr($url, 0, $pos).'/'.$querystring.'.html';*/
	}
	// echo $url;exit;
	return $urlk;
}
